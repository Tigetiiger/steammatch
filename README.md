...
|---|---|
| `/games add [game] [user]` | `game` adds one named game straight away, with autocomplete over everything this server has that you do not. Empty opens **the panel**: sync a Steam library, **Sirvi kõiki mänge** for the whole catalogue as a checklist, add any other game, or quick-add Minecraft. `user` does it **for someone else** (needs Manage Server) |
